(ns ginkgo_bioworks.main
  "Kotodama WASM entrypoint for the Ginkgo_bioworks clean-room actor (L4) — Clojure port.
  ...
  facts (`ginkgo_bioworks.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ginkgo_bioworks")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ginkgo_bioworks.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ginkgo_bioworks-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ginkgo_bioworks-compat/ready)
