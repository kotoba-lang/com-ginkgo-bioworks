(ns ginkgo_bioworks.main-test
  "Contract + behavioral test for the ginkgo_bioworks-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "ginkgo_bioworks-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'ginkgo_bioworks.main-test)]
